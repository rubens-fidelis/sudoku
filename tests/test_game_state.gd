...
func test_set_number_on_clue_does_nothing():
	...
	for i in range(81):
		if state.clues[i]:
			clue_pos = i
			break
	var original: int = state.board[clue_pos]
...
func test_set_number_clears_pencil_marks():
	...
	for i in range(81):
		if not state.clues[i]:
			empty_pos = i
			break
	state.pencil_marks[empty_pos] = (1 << 0) | (1 << 1) | (1 << 2)
	state.set_number(empty_pos, state.solution[empty_pos])
	assert_eq(state.pencil_marks[empty_pos], 0)
...
func test_set_number_zero_does_not_increment_errors():
	...
	for i in range(81):
		if not state.clues[i]:
			...
			break
	state.set_number(empty_pos, state.solution[empty_pos])
	var errors_before: int = state.error_count
...
func test_toggle_pencil_adds_mark():
	...
	assert_true(state.pencil_marks[empty_pos] & (1 << 4) != 0)
...
func test_toggle_pencil_removes_existing_mark():
	...
	state.toggle_pencil(empty_pos, 5)
	state.toggle_pencil(empty_pos, 5)
	assert_false(state.pencil_marks[empty_pos] & (1 << 4) != 0)
...
func test_toggle_pencil_on_clue_does_nothing():
	...
	assert_eq(state.pencil_marks[clue_pos], 0)
